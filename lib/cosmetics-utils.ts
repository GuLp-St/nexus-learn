...
const COSMETICS: Cosmetic[] = [
  ...
  // Custom Avatars - Unique
  { id: "avatar-hf", category: "avatar", rarity: "unique", price: 5000, name: "AI Generated", description: "A unique avatar generated by Hugging Face AI", config: { type: "custom", source: "hf" } },
  { id: "avatar-unsplash", category: "avatar", rarity: "unique", price: 5000, name: "Unsplash Pro", description: "Use any high-quality photo from Unsplash", config: { type: "custom", source: "unsplash" } },
  { id: "avatar-upload", category: "avatar", rarity: "unique", price: 5000, name: "Masterpiece Upload", description: "Upload your own masterpiece as an avatar", config: { type: "custom", source: "upload" } },
...
export async function getUserCosmetics(userId: string): Promise<UserCosmetics> {
  try {
    ...
    return {
      ...
      avatarImageKey: data.cosmetics?.avatarImageKey,
      avatarImageConfig: data.cosmetics?.avatarImageConfig,
      ownedCosmetics: {
        ...owned,
        avatars,
        themes,
...
export function resolveAvatarStyle(cosmeticId: string | undefined): AvatarStyle {
  ...
  const validStyles: AvatarStyle[] = [
    ...
    "big-smile", "avataaars", "lorelei", "micah", "hf", "unsplash", "upload"
  ]
...
export function getCosmeticPreviewUrl(cosmetic: Cosmetic, seed: string = "preview"): string | null {
  if (cosmetic.category === "avatar") {
    // Handle custom/unique avatars that don't use standard styles
    if (cosmetic.config?.type === "custom") {
      // Return a style-specific placeholder
      const source = cosmetic.config.source;
      if (source === "hf") {
        // High-quality AI looking image from Picsum with a consistent but unique seed
        return `https://picsum.photos/seed/ai-avatar-${seed}/400/400`; 
      }
      if (source === "unsplash") {
        // High-quality photography from Unsplash via Picsum
        return `https://picsum.photos/seed/photo-avatar-${seed}/400/400`;
      }
      if (source === "upload") {
        // Specialized "file" avatar using a different DiceBear style
        return `https://api.dicebear.com/7.x/shapes/svg?seed=upload-masterpiece&backgroundColor=f1f5f9&shape1Color=0ea5e9&shape2Color=38bdf8&shape3Color=7dd3fc`;
      }
    }

    if (cosmetic.config?.style) {
      try {
