...
export default function ModuleQuizPage() {
  ...
  }, [showResults, course, questions, scores, answers, moduleIndex, setPageContext])
    ...
    const loadQuiz = async () => {
      try {
        ...
        if (!allLessonsCompleted && !isChallengeMode) {
          router.push(`/quizzes/${courseId}/modules`)
          return
        }
        ...
        // If there's an incomplete quiz that isn't THIS module quiz, we'll show the blocker
        if (anyIncomplete && (anyIncomplete.courseId !== courseId || anyIncomplete.quizType !== "module" || anyIncomplete.moduleIndex !== moduleIndex)) {
          setOtherQuizInProgress(anyIncomplete)
          setLoading(false)
          return
        }

        // In challenge mode, skip resume/retake logic - always create new questions
        ...
        const searchParams = new URLSearchParams(window.location.search)
        const retake = !isChallengeMode ? searchParams.get("retake") : null
        ...
        const incompleteAttempt = !isChallengeMode ? await getIncompleteQuizAttempt(user.uid, courseId, "module", moduleIndex, null) : null

        if (incompleteAttempt && incompleteAttempt.questionIds.length > 0 && !isChallengeMode) {
        ...
        const recentAttempt = await getMostRecentQuizAttempt(user.uid, courseId, "module", moduleIndex, null)

        let availableQuestions = await fetchQuizQuestions(courseId, "module", moduleIndex, null)

        if (availableQuestions.length < QUIZ_COUNT) {
          setGenerating(true)
          const generatedQuestions = await generateModuleQuizQuestions(
            courseWithProgress,
            moduleIndex,
            courseId,
            QUIZ_COUNT * 2
          ...
          availableQuestions = await fetchQuizQuestions(courseId, "module", moduleIndex, null)
        }
