...
import { ArrowLeft, CheckCircle2, X, Star, Clock, XCircle, FileQuestion } from "lucide-react"
...
export default function QuizHistoryPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        console.log(`[QuizHistory] Fetching history for course: ${courseId}`)
        ...
        console.log(`[QuizHistory] Found ${allAttempts.length} attempts`)
                  ...
                  return (
                    <Card 
                      ...
                      onClick={async () => {
                        ...
                        setSelectedAttemptQuestions([]) // Reset questions
                        setLoadingQuestions(true)
                        setQuestionError("")
                        
                        // Emit review quiz event
                        const { emitQuestEvent } = require("@/lib/event-bus")
                        emitQuestEvent({
                          type: "quest.review_quiz",
                          userId: user!.uid,
                        ...
                        if (attempt.questionIds && attempt.questionIds.length > 0) {
                          try {
                            ...
                            console.error("Error loading questions for attempt:", error)
                          ...
                          setLoadingQuestions(false)
                          setQuestionError("No questions recorded for this attempt.")
                        }
                      }}
                    >
                ...
                <div className="space-y-3">
                  ...
                  {questionError && (
                    <div className="text-center py-8 bg-muted/50 rounded-lg border-2 border-dashed">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">This usually happens for older quiz attempts where data wasn't saved.</p>
                    </div>
                  )}
