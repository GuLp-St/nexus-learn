...
import { ArrowLeft, CheckCircle2, XCircle, ChevronRight, ChevronLeft, Star, Clock } from "lucide-react"
...
export default function ModuleQuizPage() {
    ...
    if (course && user) {
      ...
      setPageContext({
        title: showResults && course
          ...
          : course
          ? `Taking Module Quiz: ${course.title} - Module ${moduleIndex + 1}`
          : "Module Quiz",
  ...
  }, [course, user, showResults, questions, scores, answers, moduleIndex, currentQuestionIndex, setPageContext])
  ...
  // Auto-start quiz in challenge mode (if applicable)
  useEffect(() => {
    if (isChallengeMode && course && !loading && !generating && !questions.length && user && !quizStartedRef.current) {
      ...
      const timeout = setTimeout(() => {
        ...
        // loadQuiz will handle this, let's just make sure it's consistent
      }, 200)
  ...
  useEffect(() => {
    ...
    const loadQuiz = async () => {
      ...
      try {
        ...
        // Check if all lessons in this module are completed (skip in challenge mode)
        const moduleLessonIds = module.lessons.map((_, idx) => `${moduleIndex}-${idx}`)
        const allLessonsCompleted = moduleLessonIds.every(id => 
          courseWithProgress.userProgress?.completedLessons?.includes(id)
        )

        if (!allLessonsCompleted && !isChallengeMode) {
          router.push(`/journey/${courseId}`)
          return
        }
        ...
        if (!isChallengeMode) {
          ...
          if (activeQuiz && (activeQuiz.courseId !== courseId || activeQuiz.quizType !== "module" || Number(activeQuiz.moduleIndex) !== moduleIndex)) {
          ...
          // If active quiz matches this quiz, resume it
          if (activeQuiz && activeQuiz.id && activeQuiz.courseId === courseId && activeQuiz.quizType === "module" && Number(activeQuiz.moduleIndex) === moduleIndex) {
            quizStartedRef.current = true
            const availableQuestions = await fetchQuizQuestions(courseId, "module", moduleIndex, null)
            const questionMap = new Map(availableQuestions.map(q => [q.questionId, q]))
            const restoredQuestions = activeQuiz.questionIds
              .map(id => questionMap.get(id))
              .filter((q): q is QuizQuestion => q !== undefined)

            if (restoredQuestions.length > 0 && activeQuiz.id) {
      ...
      if (attemptId && user) {
        ...
        if (attemptDoc.exists()) {
          ...
          // For now, treat as normal quiz
          {
            ...
            // Save highest score to module progress
            ...
            // Update module quiz score in user progress
            const progressRef = doc(db, "userCourseProgress", `${user.uid}-${courseId}`)
            const progressDoc = await getDoc(progressRef)
            if (progressDoc.exists()) {
              const currentData = progressDoc.data()
              const moduleQuizScores = currentData.moduleQuizScores || {}
              const currentHighest = moduleQuizScores[moduleIndex] || 0
                            ...
                            {/* Citations - Only show if facts are found */}
                                {(() => {
                                  ...
                                  const foundFacts = factIds.map((factId) => {
                                    // Find fact in current module first, then across all modules
                                    const module = course.modules[moduleIndex]
                                    let found = (module as any).accumulatedContext?.find((f: any) => f.id === factId)
                                    if (found) return found
                                    
                                    // Search other modules if not found
                                    for (let i = 0; i < course.modules.length; i++) {
                                      const mod = course.modules[i]
                                      found = (mod as any).accumulatedContext?.find((f: any) => f.id === factId)
                                      if (found) return found
                                    }
                                    return null
                                  }).filter(Boolean)
