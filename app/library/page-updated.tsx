...
function ContinueLearningButton({ courses }: { courses: CourseWithProgress[] }) {
  ...
  useEffect(() => {
    const findContinueUrl = async () => {
      ...
      // Find the most recent course with progress < 100%
      const activeCourses = courses.filter((c) => (c.userProgress?.progress || 0) < 100)
...
const CourseCard = ({ course, index }: { course: CourseWithProgress; index: number }) => {
              ...
              {addedCourses.length === 0 ? (
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  {addedCourses.map((course, index) => (
                    <CourseCard key={course.id} course={course} index={index + myCourses.length} />
                  ))}
                </div>
              )}
            </div>
